...
export interface UlyssesPageProps extends BasePageProps {
    config: ProsperoConfig;
    galaxyFold: PagesOutput;
    iphoneXR: PagesOutput;
}

export async function getStaticProps(): Promise<{ props: UlyssesPageProps }> {
    const props = await getBaseProsperoProps();

    const loaderBuilder = await LoaderBuilder.fromFile(
        join(cwd(), './src/prospero/texts/ulysses.txt')
    );
